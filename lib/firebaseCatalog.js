...
function formatMedicineForStore(med, imageMap) {
  ...
  const stock = Number(doc.stock ?? doc.quantity_available ?? 0) || 0;
  ...
  const formatted = {
    ...
    orderable: storeVisible && stock > 0 && price > 0,
    storeVisible
  };
...
async function getMedicinesByIds(ids = []) {
  ...
  const byId = new Map();
  for (const med of cache.medicines) {
 ...
 * Replaces client prices with catalog prices and enforces stock limits.
 */
async function validateOrderItemsAgainstCatalog(items = []) {
  ...
  for (const item of items) {
    ...
    const catalogMed = findCatalogMedicineById(cache.medicines, medicineId);
    if (!catalogMed) {
      const err = new Error(
    ...
    const stock = Number(catalogMed.stock || 0);
    ...
    if (stock <= 0) {
      const err = new Error(`Out of stock: ${catalogMed.name}`);
      err.status = 400;
      throw err;
    }
    ...
    if (qty > stock) {
      const err = new Error(`Insufficient stock for ${catalogMed.name} (max ${stock})`);
      err.status = 400;
      throw err;
    }
...
async function getMedicinesPaginated(opts = {}) {
  ...
  const list = filterMedicines(cache.medicines, opts);
