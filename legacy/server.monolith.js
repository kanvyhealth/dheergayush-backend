...
const { connectDatabase, requireDb, isConnected, getProvider } = require('./src/core/db');
const {
  syncStoreCatalogFromImages,
  getStoresFromDatabase
} = require('./src/modules/store/storeCatalog');
const { getEffectiveStatus, normalizeDbStatus, getScheduleStatus } = require('./src/modules/doctors/doctorAvailability');
const {
  ...
  isDoctorAvailable,
  isDoctorApproved
} = require('./src/modules/doctors/doctorPresence');
const {
  verifyIdToken,
  syncUserFromToken,
  getUserProfile,
  requireFirebaseAuth
} = require('./src/core/firebase/auth');
const { generateVideoRoomId, resolveFileUrl, uploadFile } = require('./src/core/firebase/storage');
const { enrichDoctorPhotos, streamDoctorPhoto } = require('./src/modules/doctors/doctorPhotoUrl');
const { uploadToFirebase, saveDocumentRecord } = require('./src/core/uploads');
const {
...
} = require('./src/core/firebase/authRest');
const { initFirebase, getAdmin } = require('./src/core/firebase');
const {
...
const { getFirestore } = require('./src/core/firebase');
const {
  validateCredentials: validateAdminCredentials,
  issueAdminToken,
  revokeAdminToken,
  requireAdmin
} = require('./src/modules/admin/adminAuth');
const {
...
const { generateAgoraToken } = require('./src/modules/consultations/agoraToken');
const {
  ...
  normalizePhone
} = require('./src/modules/payments/paymentLookup');
const {
  getPatientDiagnosisHistoryForDoctor,
  saveConsultationClinicalNotes
} = require('./src/modules/patients/patientDiagnosisHistory');
const { createPrescriptionStoreOrder } = require('./src/modules/prescriptions/prescriptionCheckout');
const {
...
} = require('./src/core/workflowAuth');
const { applyDoctorStorePricing } = require('./src/modules/doctors/doctorStoreDiscount');
const {
...
} = require('./src/modules/doctors/doctorPaymentDetails');
const {
...
} = require('./src/modules/doctors/doctorFeeApproval');
const { findCustomerByPhone, findCustomerByUid, listCustomers, findDoctorByUid, findDoctorById, findDoctorByEmail, listDoctors } = require('./src/core/userQueries');
const {
  getPublicKeyId,
  createOrder,
...
const { getCheckoutDisplayConfig, isMobileUserAgent } = require('./src/modules/payments/razorpayCheckoutConfig');
const { injectPageSeo } = require('./src/core/seoMeta');
const { resolveReportEntries } = require('./src/core/reportUrls');
const {
  initRealtime,
  emitDoctorStatus,
...
} = require('./src/modules/consultations/consultationSessionCleanup');
const { refundConsultationForRoom, refundCapturedRazorpayPayment } = require('./src/modules/consultations/consultationRefund');
const {
  findActiveAccess,
  ...
  canUseFreeFollowUp,
  FREE_FOLLOWUP_LIMIT
} = require('./src/modules/consultations/consultationAccess');
const {
  resolvePatientUid,
...
} = require('./src/modules/consultations/consultationLifecycleSync');
const { updateActiveCallForAppointment } = require('./src/modules/consultations/activeCallSync');
const {
  DEFAULT_WORKING_DAYS,
  DEFAULT_WORKING_DAYS_INT,
  parseAvailableTimeToWorkingHours,
  workingDaysToAppFormat
} = require('./src/modules/doctors/doctorSchedule');
const { mirrorDoctorToAuthUid, syncAllDoctorMirrors } = require('./src/modules/doctors/doctorMirror');
const { ensureDoctorPublicId } = require('./src/modules/doctors/doctorPublicId');
const { linkAppointmentsToAuthUid } = require('./src/modules/patients/patientLinking');
const {
  buildSharedOrderId,
  buildFirestoreOrderPayload
} = require('./src/modules/store/webOrderSync');
...
app.get('/api/health', async (req, res) => {
  const { verifyFirestoreRead, hasServiceAccount } = require('./src/core/firebase');
...
app.get('/api/firebase/collections', async (req, res) => {
  try {
    const { initFirebase } = require('./src/core/firebase');
    await initFirebase();
...
app.post('/api/register-doctor', upload.fields([
    ...
    try {
        ...
        const doctorPayload = {
            ...
            ...require('./src/modules/doctors/doctorFields').buildApprovalFirestorePatch('pending'),
            ...require('./src/modules/doctors/doctorFields').buildWorkingFirestorePatch('offline'),
...
function enrichDoctorRow(d) {
    const { enrichDoctorApiFields } = require('./src/modules/doctors/doctorFields');
...
app.put('/api/admin/doctors/:id/status', async (req, res) => {
    try {
        ...
        const { validateApprovalTransition } = require('./src/modules/doctors/doctorFields');
...
app.get('/api/admin/settlements', async (req, res) => {
    try {
        ...
        } = require('./src/modules/doctors/doctorSettlement');
        const payments = await Payment.find({}).sort({ createdAt: -1 }).exec();
        const doctors = await listDoctors();
...
app.put('/api/admin/doctors/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { parseAdminDoctorUpdates, validateApprovalTransition } = require('./src/modules/doctors/doctorFields');
...
async function markCallDisconnected(roomId, options = {}) {
    const {
      ...
      buildSessionState,
      DISCONNECT_GRACE_MS
    } = require('./src/modules/consultations/callDisconnectGrace');
    const ctx = await loadRoomContext(roomId);
...
async function clearCallDisconnectGrace(roomId) {
    const { buildClearGracePatch } = require('./src/modules/consultations/callDisconnectGrace');
    const ctx = await loadRoomContext(roomId);
    if (!ctx?.consultation) return false;
    const id = ctx.consultation._id || ctx.consultation.id;
    if (!id) return false;
...
app.post('/api/video-room/:roomId/consultation-complete-answer', async (req, res) => {
    try {
        const { RECONNECT_RING_MS } = require('./src/modules/consultations/callDisconnectGrace');
...
app.post('/api/video-room/:roomId/call-disconnected', async (req, res) => {
    try {
        const { DISCONNECT_GRACE_MS } = require('./src/modules/consultations/callDisconnectGrace');
