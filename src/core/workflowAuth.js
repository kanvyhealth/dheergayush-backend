const { verifyIdToken, getUserProfile } = require('./firebase/auth');
const { findCustomerByUid, findCustomerByPhone, findDoctorByUid, findDoctorByEmail, normalizePhone } = require('./userQueries');
...
async function resolveFirebaseSession(req) {
  ...
  try {
    ...
    let doctor = await findDoctorByUid(decoded.uid);
    if (!doctor && profile?.email) {
      doctor = await findDoctorByEmail(profile.email);
    }
    const customer =
