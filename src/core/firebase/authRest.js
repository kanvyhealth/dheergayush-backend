 ...
 */
const { getAdmin, initFirebase } = require('./');
...
async function sendOobCode(payload) {
  ...
  const data = await parseIdentityToolkitResponse(res);
  if (!res.ok) {
    const msg = data.error?.message || 'Email action failed';
    const err = new Error(msg.replace(/_/g, ' ').toLowerCase());
    err.code = data.error?.message || 'EMAIL_ACTION_FAILED';
    throw err;
  }
  return data;
}

async function sendEmailVerification(idToken, options = {}) {
  if (!idToken) throw new Error('idToken is required to send verification email');
  const payload = {
    requestType: 'VERIFY_EMAIL',
    idToken
  };
  if (options.continueUrl) payload.continueUrl = options.continueUrl;
  return sendOobCode(payload);
}
