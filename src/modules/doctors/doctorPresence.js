...
const { Doctor } = require('../../core/data');

const { findDoctorByName } = require('../../core/userQueries');
...
async function syncDoctorRecordsUpdate(doctor, patch) {
  ...
  const { sanitizeDoctorFeeUpdate, isTrustedFeeWrite } = require('./doctorFeeApproval');
  let payload = { ...patch };
  if (!isTrustedFeeWrite(payload)) {
    const sanitized = sanitizeDoctorFeeUpdate(doctor, payload);
    payload = sanitized.$set ? { ...sanitized.$set } : { ...sanitized };
  } else {
  ...
  let updated = null;
  for (const id of ids) {
    updated = await Doctor.findByIdAndUpdate(id, { $set: payload }, { new: true });
  }
...
async function deleteAllDoctorRecords(doctor) {
  const ids = await collectDoctorDocumentIds(doctor);
  for (const id of ids) {
