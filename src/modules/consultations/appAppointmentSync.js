...
const { ensurePatientUid } = require('../patients/patientLinking');
...
/** Map consultation_* / bare appointment aliases to room_* for Agora (never join consultation_*). */
function canonicalizeAgoraChannelAlias(requestedChannelName = '', appointmentId = '') {
  const requested = String(requestedChannelName || '').trim();
  const id = String(appointmentId || '').trim();
  if (requested.startsWith('consultation_')) {
    const fromCall = requested.slice('consultation_'.length).trim();
    return fromCall ? videoRoomIdForAppointment(fromCall) : (id ? videoRoomIdForAppointment(id) : requested);
  }
  if (id && (!requested || requested === id || requested === scheduledCallId(id))) {
    return videoRoomIdForAppointment(id);
  }
  if (requested && !requested.startsWith('room_') && id && requested === id) {
    return videoRoomIdForAppointment(id);
  }
  return requested || (id ? videoRoomIdForAppointment(id) : '');
}

function canonicalVideoChannelForAppointment(appointment, appointmentId, requestedChannelName = '') {
  const id = String(appointmentId || appointment?._id || appointment?.id || '').trim();
  const roomId = String(
    appointment?.videoRoomId ||
      appointment?.roomId ||
      (id ? videoRoomIdForAppointment(id) : '')
  ).trim();
  const requested = String(requestedChannelName || '').trim();
  if (!roomId) return canonicalizeAgoraChannelAlias(requested, id);

  const aliases = new Set([
    roomId,
    id,
    id ? scheduledCallId(id) : '',
    id ? videoRoomIdForAppointment(id) : '',
    appointment?.activeCallId || '',
    appointment?.callId || '',
    appointment?.callRoomId || ''
  ].filter(Boolean).map(String));

  if (!requested || aliases.has(requested)) return roomId;
  return canonicalizeAgoraChannelAlias(requested, id) || requested;
}
...
function buildActiveCallRecord({ appointmentId, appointment, doctorId, patientId }) {
  ...
  const docId = doctorId || (appointment && appointment.doctorId) || '';
  const patId = patientId || (appointment && (appointment.patientId || appointment.userId)) || '';
  const now = new Date();
  return {
    _id: id,
    callId: id,
    ...
    caller: docId,
    callee: patId,
    participants: [docId, patId].filter(Boolean),
    status: 'ringing',
    callType: 'video',
    scheduledConsultation: true,
    callRoomId: roomId,
    roomId,
    videoRoomId: roomId,
    channelName: roomId,
