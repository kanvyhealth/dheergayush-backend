 ...
 */
const { ConsultationRequest, Payment } = require('../../core/data');
const { normalizeConsultationStatus, buildConsultationStatusFields } = require('./consultationWorkflow');
const { findDoctorByName, updateDoctorPresence } = require('../doctors/doctorPresence');
const { buildStatusPayload, emitDoctorStatus } = require('../../core/realtime');

const STALE_ACTIVE_MS = 45 * 60 * 1000;
const VIDEO_CALL_INACTIVITY_MS = 60 * 1000;
const { inCallStaleThresholdMs, isWithinDisconnectGrace, isReconnectRingActive } = require('./callDisconnectGrace');
...
function staleThresholdForStatus(status, maxAgeMs = STALE_ACTIVE_MS, consultation = null) {
  if (consultation) {
    const graceThreshold = inCallStaleThresholdMs(consultation);
    if (graceThreshold) return graceThreshold;
  }
...
function isStaleActiveConsultation(consultation, maxAgeMs = STALE_ACTIVE_MS) {
  ...
  let threshold = staleThresholdForStatus(status, maxAgeMs, consultation);
  if (consultation.callDisconnectedAt) threshold = inCallStaleThresholdMs(consultation) || threshold;
...
async function markConsultationCompletedLocal(consultation) {
  ...
  if (id) {
    ...
    const { syncPaymentForConsultationStatus, syncActiveCallForStatus } = require('./consultationLifecycleSync');
    await syncPaymentForConsultationStatus(consultation.paymentId, 'completed', id);
