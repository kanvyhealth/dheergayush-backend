const { Payment, Doctor, User, ConsultationRequest, Order } = require('../../core/data');
const { normalizePaymentForWeb } = require('../../core/mobileSchema');
const { formatConsultationResponse } = require('../consultations/consultationWorkflow');
const { normalizePhone, listCustomers, listDoctors } = require('../../core/userQueries');
...
async function listConsultationHistoryForPatient(phoneOrUid) {
  const lookup = await loadLookupCache();
  const payments = await listPaymentsForPatient(phoneOrUid);
  const appointments = await ConsultationRequest.find({}).sort({ createdAt: -1 });
  const byRoom = new Map();
  ...
  for (const p of payments) {
    ...
    const entry = {
      ...
      paymentId: id,
      consultationId: p.consultationId ? String(p.consultationId) : '',
      doctorName: p.selectedDoctorName || p.doctorName || '—',
      amount: Number(p.amount || p.selectedDoctorFee || 0),
      status: normalizeHistoryStatus(p.consultationStatus || p.status),
      roomId: room,
      createdAt: p.createdAt || null,
  ...
  const customer =
    lookup.customerByKey.get(String(phoneOrUid)) ||
    lookup.customerByKey.get(needlePhone);
  const patientUid = customer?.uid || customer?._id;

  for (const raw of appointments) {
    const c = formatConsultationResponse(raw);
    const consultPhone = normalizePhone(c.patientPhone || raw.patientPhone || '');
    const consultUid = String(c.patientId || raw.patientId || raw.uid || '');
    const phoneMatch = needlePhone && consultPhone && consultPhone === needlePhone;
    const uidMatch = patientUid && consultUid && String(patientUid) === consultUid;
    const idMatch = String(phoneOrUid).length > 10 && consultUid === String(phoneOrUid);
    if (!phoneMatch && !uidMatch && !idMatch) continue;
    const room = c.roomId || c.videoRoomId || '';
    const consultId = String(c._id || c.id || '');
    const existing = room ? byRoom.get(room) : null;
    if (existing) {
      ...
      existing.status = normalizeHistoryStatus(c.status || c.consultationStatus || existing.status);
      if (c.amount != null) existing.amount = Number(c.amount);
      if (c.doctorName) existing.doctorName = c.doctorName;
      if (c.doctorAvailableTime) existing.doctorAvailableTime = c.doctorAvailableTime;
      if (!existing.createdAt && c.createdAt) existing.createdAt = c.createdAt;
    } else {
      const entry = {
...
function orderMatchesPatient(raw, phoneOrUid, lookup) {
  const needlePhone = normalizePhone(phoneOrUid);
  const customer =
    lookup.customerByKey.get(String(phoneOrUid)) ||
    lookup.customerByKey.get(needlePhone);
  const uid = customer?.uid || customer?._id;
  const phone = normalizePhone(raw.customerPhone || raw.phone || raw.patientPhone || '');
  const keys = [raw.userId, raw.patientId, raw.uid].filter(Boolean).map(String);
  if (needlePhone && phone === needlePhone) return true;
  if (uid && keys.includes(String(uid))) return true;
  if (String(phoneOrUid).length > 10 && keys.includes(String(phoneOrUid))) return true;
  return false;
}

async function listOrdersForPatient(phoneOrUid) {
  const lookup = await loadLookupCache();
  const all = await Order.find({}).sort({ orderDate: -1 });
  return all
    .filter((o) => orderMatchesPatient(o, phoneOrUid, lookup))
    .map((o) => {
      ...
      return {
        ...
        orderStatus: d.orderStatus || d.deliveryStatus || 'pending',
        source: d.source || 'website',
        prescriptionId: d.prescriptionId || '',
        orderDate: d.orderDate || d.createdAt || null,
        createdAt: d.createdAt || d.orderDate || null
      };
    });
}
