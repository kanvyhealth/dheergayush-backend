...
const { normalizePhone } = require('../../core/userQueries');
...
async function ensurePatientUid({ phone, name, email, firebaseUid }) {
  ...
  const { findCustomerByPhone } = require('../../core/userQueries');
...
async function linkAppointmentsToAuthUid({ authUid, phone }) {
  ...
  for (const oldId of idsToReplace) {
    ...
    const accessRows = await db
      .collection('consultation_access')
      .where('patientId', '==', oldId)
      .get();
    for (const doc of accessRows.docs) {
      await doc.ref.set(
        {
          patientId: authUid,
          patientUid: authUid,
    ...
    for (const doc of activeCalls.docs) {
      await doc.ref.set({ patientId: authUid, updatedAt: new Date() }, { merge: true });
      updated += 1;
    }
    ...
    for (const doc of documents.docs) {
      await doc.ref.set(
        { patientId: authUid, userId: authUid, updatedAt: new Date() },
        { merge: true }
      );
      updated += 1;
    }
